...
(def test-parsed-score
  [{1 ['(begin :my-gesture-name
               {:type :ssg, :spec {:instr test-synth, :freq 200}})
       '(begin :other-gesture-name
               {:type :ssg, :spec {:instr test-synth, :freq 400}})]}])

(def test-mutations
  {:my-gesture-name
   [{:measure 1, :quant 1, :timestamp 0N,
     :spec [:spec {:instr 'test-synth, :freq 200}]}
    {:name :my-gesture-name,
...
(facts "about `zip-mutations`"
       (fact
        (let [score test-parsed-score
          ...
          [{1 ['(begin :my-gesture-name
                       {:type :ssg,
                        :spec {:instr test-synth, :freq 200}}
                       [{:measure 1, :quant 1, :timestamp 0N,
                         :spec [:spec {:instr test-synth, :freq 200}]}
                        {:name :my-gesture-name,
